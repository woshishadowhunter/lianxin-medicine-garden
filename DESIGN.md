...
选定方向为“自然观察手账”。设计以克制的纸张层次、墨绿主操作、宋体品牌标题和家庭真实植物照片建立记录可信度；花卉、绿植、蔬果和本草共享同一套信息层级。

## Design Image Workflow

v2 视觉方向记录在 `docs/superpowers/specs/2026-07-12-all-plants-expansion-design.md`。新页面沿用现有全局 token，并以真实养护照片优先于装饰插画。
...
继续使用 `miniprogram/components/lx-icon` 与 `/images/ui-icons/<tone>/<name>.png`。图标应为统一线性/实心混合风格，单屏最多使用 2 个主要色调。植物状态、养护类型、导航和操作图标不得回退到 emoji。

## Component Grammar

- Home Journal：完整显示“连心植物园”，携带家庭编号、社区、积分和最近真实成长照片。
- Priority Action Bar：新增养护是家庭端最强动作，记录和药园为次动作。
- Evidence Module：照片网格、记录卡、审核详情都以照片为第一证据。
- Task Row：植物任务以行项目为主，显示名称、类别、状态、进度或陪伴天数、上次养护和关注提示。
...
4. 我的植物园：按类别筛选植物任务，使用清晰任务行并突出关注状态。
